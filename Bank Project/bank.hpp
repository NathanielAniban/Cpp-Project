...
string Database3Path = "C:\\Users\\TECH\\Desktop\\C++ Projects\\Bank Project\\output\\Transaction\\"; //Transaction Path
class BankInfo{
    public:
    ...
    void EditInfo(){
        ...
        for(int x = 0; x < counter; x++){
            if(id == ID_Getter[x]){
                ...
                string OldTransactionPath = Database3Path
                 + id + name + Database3;
                char *OldPath = &OldTransactionPath[0];
                name = Name;
                string NewTransactionPath = Database3Path
                 + id + name + Database3;
                char *NewPath = &NewTransactionPath[0];
